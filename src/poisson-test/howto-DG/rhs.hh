...
void assembleRHS ( const Function &function, DiscreteFunction &rhs )
{
	...
	std::ofstream fFtest ("F-test-orig.dat");
	...
	for( IteratorType it = dfSpace.begin(); it != end; ++it )
	{
		...
		for( size_t pt = 0; pt < numQuadraturePoints; ++pt )
		{
			...
			const unsigned int nDof = rhsLocal.numDofs();


			// multiply by quadrature weight
			f *= quadrature.weight( pt ) * geometry.integrationElement( x );
			fFtest << geometry.center() << " " << quadrature.weight( pt ) << " " << f << std::endl;
