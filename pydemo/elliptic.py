...
errfile.write("%s\n" % ('ndof, l2err, h1err, LinIter, NewtnIter') ) #(spc.size, errors[0], errors[2], info["linear_iterations"], info["iterations"]))
def solve(grid,model,exact,space,scheme,spaceKwargs,order):
    ...
    spaceSize += [spc.size]
    errfile.write("%i,%e,%e,%i,%i\n" % (spc.size, errors[0], errors[2], info["linear_iterations"], info["iterations"]))
    return interpol, df
...
for i in range(endEoc-start):
    ...
    N = 2*n
    print("Test: ",n,N)
    constructor = cartesianDomain([0.0001,0.0001],[1,1],[N,N])
