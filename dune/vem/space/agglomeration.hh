...
namespace Dune {
    ...
    namespace Vem {
        ...
        public:
            ...
            struct InterpolationType {
                ...
                void operator()(const U &u, V &v) { inter_(element_, u, v); }
            ...
            const BasisFunctionSetType basisFunctionSet(const EntityType &entity) const {
              ...
              typename Traits::ScalarBBBasisFunctionSetType bbScalarBasisFunctionSet(entity, bbox,
                                                                                     useOnb_,
                                                                                     std::move(scalarShapeFunctionSet));
              // vectorial extended VEM Basis
              typename Traits::ScalarBasisFunctionSetType scalarBFS(entity, bbox, valueProjection, jacobianProjection,
                                                                    hessianProjection,
            ...
            const typename Traits::ScalarBasisFunctionSetType scalarBasisFunctionSet(const EntityType &entity) const {
              ...
              typename Traits::ScalarBBBasisFunctionSetType bbScalarBasisFunctionSet(entity, bbox,
                                                                                     ...
                                                                                     std::move(scalarShapeFunctionSet));
              // vectorial extended VEM Basis
              return typename Traits::ScalarBasisFunctionSetType(entity, bbox, valueProjection, jacobianProjection,
                                                                 hessianProjection,
                                                                 std::move(bbScalarBasisFunctionSet));
            }
            ...
            const Stabilization &
            stabilization(const EntityType &entity) const { return stabilizations_[agglomeration().index(entity)]; }
        ...
        inline void AgglomerationVEMSpace<FunctionSpace, GridPart, polOrder>::buildProjections() {
          ...
          for (const ElementType &element : elements(static_cast< typename GridPart::GridViewType >( gridPart()),
                                                     Partitions::interiorBorder))
        ...
        const BasisFunctionSetType basisFunctionSet(const EntityType &entity) const {
          ...
          // vectorial extended VEM Basis
          typename Traits::ScalarBasisFunctionSetType scalarBFS(entity, bbox, valueProjection, jacobianProjection,
                                                                hessianProjection, std::move(bbScalarBasisFunctionSet));
          return BasisFunctionSetType(std::move(scalarBFS));
        }
    ...
    inline void AgglomerationVEMSpace<FunctionSpace, GridPart, polOrder>::buildProjections() {
      ...
      //!TS Where orders.size() is always 2, i.e., for evaluate, jacobian, hessian
      std::vector<int> orders = agIndexSet_.orders();
      >>>>>>> 9d218997b6deb4de35b81ce35f1de0999e1990c9
      ...
      for (std::size_t agglomerate = 0; agglomerate < agglomeration().size(); ++agglomerate) {
        ...
        for (const ElementSeedType &entitySeed : entitySeeds[agglomerate]) {
          ...
          for (std::size_t qp = 0; qp < quadrature.nop(); ++qp) {
            ...
            shapeFunctionSet.evaluateEach(quadrature[qp],
                                          [&](std::size_t alpha, FieldVector<DomainFieldType, 1> phi) {
                                              shapeFunctionSet.evaluateEach(quadrature[qp], [&](std::size_t beta,
                                                                                                FieldVector<DomainFieldType, 1> psi) {
                                                  Hp[alpha][beta] += phi[0] * psi[0] * weight;
                                                  if (alpha < numGradShapeFunctions &&
                                                      beta <
                                                      numGradShapeFunctions) // basis set is hierarchic so we can compute HpGrad using the order p shapeFunctionSet
            ...
            for (std::size_t qp = 0; qp < quadrature.nop(); ++qp) {
              ...
              shapeFunctionSet.evaluateEach(y, [&](std::size_t alpha, FieldVector<DomainFieldType, 1> phi) {
                  if (alpha < numGradShapeFunctions)
                    ...
                    edgeShapeFunctionSet_.evaluateEach(x,
                                                       [&](std::size_t beta, FieldVector<DomainFieldType, 1> psi) {
                                                           if (beta <
                                                               edgePhiVector[0].size()) //assemble left hand side here for ls problem
                                                             for (int s = 0; s <
                                                                             mask[0].size(); ++s) // note that edgePhi is the transposed of the basis transform matrix
                                                               R[alpha][mask[0][s]].axpy(
                                                                       edgePhiVector[0][beta][s] * psi[0] * phi[0] *
                                                                       weight, normal);
                                                       });
                    ...
                    edgeShapeFunctionSet_.evaluateEach(x,
                                                       [&](std::size_t beta, FieldVector<DomainFieldType, 1> psi) {
                                                           if (beta < edgePhiVector[1].size())
                                                             for (int s = 0; s <
                                                                             mask[1].size(); ++s) // note that edgePhi is the transposed of the basis transform matrix
                                                               P[alpha][mask[1][s]].axpy(
                                                                       edgePhiVector[1][beta][s] * psi[0] * phi[0] *
                                                                       weight, factorNN);
                                                       });
